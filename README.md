...
A ogni tick i setup vengono valutati in cascata — **mtf-continuation-v1** → **`m1_short`** → **`m1_range`** → **`quick_tick`** (solo con `exit_mode=fast`) — e vince il primo che produce un ordine; la posizione aperta resta comunque una sola. Prima viene valutata la mtf (M15 → M5 → M1, descritta sotto). Solo se non produce nulla viene valutato il secondo setup **`m1_short`** (`m1-short-v1`), che guarda l'M1 per il trigger ma solo dopo aver letto il contesto M5/M15:
...
`npm run scenarios` esegue scenari deterministici offline (mtf con `SHORT_ENABLED=false`, più un blocco dedicato al `m1_short`) per BUY/SELL, M15 in range, M5 pullback/retest, conferma M1, dati mancanti, costi, stop, ripetizione dei preflight, recupero ordini e rischio con tick value, più il gate M15 a 4 stati (`scripts/m15-gate-scenarios.ts`, inclusa la non-regressione con `M15_GATE_MODE=off` e una matrice di 72 combinazioni M5×M15), SL/TP da struttura (`scripts/sltp-scenarios.ts`, `SLTP_MODE=fixed`/`trailing`/`off`) e liquidità di mercato (`scripts/market-liquidity-scenarios.ts`: warmup, potatura delle finestre, i tre livelli da spread/tick-rate/ATR, soglie configurabili, lotti e setup ammessi per livello). Non chiama MetaApi né il database.

### Liquidità di mercato (`src/lib/server/marketLiquidity.ts`)

Il livello di liquidità del momento viene da segnali di mercato reali, non dall'ora dell'orologio: uno spread anomalo, un tick-rate crollato o un ATR M1 fuori scala rispetto all'M15 raccontano una liquidità sottile molto meglio di una fascia oraria fissa, che non si accorge di una news o di una chiusura anticipata dentro il suo stesso orario "normale". Dentro la fascia `SCALPER_HOURS_UTC` già esistente (non la tocca, né il flatten di fine fascia):

- **`LOW`** se lo spread corrente supera `SPREAD_RATIO_LOW` (2.0) volte la media della finestra (`SPREAD_MA_WINDOW_MIN`, 30 min), **oppure** il tick-rate recente (`TICK_RATE_WINDOW_SEC`, 60s) scende sotto `TICK_RATE_LOW_PCT` (40%) di quello atteso dalla baseline (`TICK_RATE_BASELINE_MIN`, 60 min), **oppure** l'ATR M1/M15 supera `ATR_RATIO_HIGH_NOISE` (1.8): solo `mtf` resta attivo, `m1_short` e `m1_range` escono dalla valutazione, lotti × `LOT_MULT_LOW` (default 0.5).
- **`HIGH`** solo se spread ≤ `SPREAD_RATIO_HIGH` (1.3) **e** tick-rate ≥ 80% dell'atteso **e** ATR M1/M15 ≤ 1.2: tutti e tre i setup attivi, lotti invariati.
- **`MEDIUM`** in tutti gli altri casi, e sempre durante il warmup (worker avviato da meno di `WARMUP_MIN`, 15 min, o meno di 10 campioni di spread ancora raccolti): mai un livello inventato da una storia troppo corta.

Lo stato (storia dello spread, timestamp dei tick, ultimo ATR M1/M15) vive per tutta la vita del processo worker, aggiornato ad ogni tick; l'ATR arriva dalla stessa funzione `atr()` già usata dalla strategia, mai ricalcolato da zero. I lotti finali passano comunque da `clampLots`, la stessa funzione usata ovunque nel worker, quindi restano sempre entro `SCALPER_LOTS_MIN`/`SCALPER_LOTS_MAX` e arrotondati al passo lotti del broker. Il livello calcolato e i tre rapporti esatti (spread/tick-rate/ATR) compaiono come voce `market_liquidity` in `stream_last_decision.evaluations` ad ogni tick e nel messaggio Telegram di ogni apertura.
